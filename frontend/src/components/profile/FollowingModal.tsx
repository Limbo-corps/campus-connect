...
import { useFollowing, followUser, unfollowUser } from "@/hooks/useFollow";
...
export default function FollowingModal({
  ...
  const { following, loading, mutate: mutateFollowing } = useFollowing(userId);
  ...
  const handleFollowToggle = async (targetUser: User) => {
    ...
    try {
      ...
      await Promise.all([mutateMyFollowing(), mutateFollowing()]);
    } catch (err) {
      console.error("Failed following relationship toggle:", err);
    } finally {
              ...
              <div className="flex flex-col gap-3 h-auto max-h-87.5 overflow-y-auto overflow-x-hidden pr-1">
                {loading ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center">
                    ...
                    >
                      Campus accounts followed by this user will be listed here.
                    </Typography.Paragraph>
                  </div>
                ) : (
                  following.map((user) => {
