 ...
 */
export default function CollapsedRail({ onExpand }: Props) {
  ...
  return (
    ...
    >
      ...
      <Link
        ...
        className={`my-1 rounded-full transition-transform hover:scale-105 ${
          !username ? "pointer-events-none opacity-80" : ""
        }`}
      >
      ...
      {NAV.map(({ href, icon: Icon, label, badge, disabled }) => {
        ...
        return (
          ...
          >
            {active && (
              <span className="absolute left-0 h-5 w-1 rounded-r-full bg-white" />
            )}
