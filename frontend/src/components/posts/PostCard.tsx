...
import { Heart, MessageCircle, Share2, Pencil, X } from "lucide-react";
...
import UserCardHeader from "./UserCardHeader"; // Import the new separated component
  ...
  return (
    <>
      <Card className="w-full overflow-hidden border border-[--surface-secondary] shadow-sm transition-shadow hover:shadow-md">
        {/* Header - Rendering our cleanly separated user card component */}
          ...
          {post.title &&
            (detail ? (
              ...
              <Link href={`/posts/${post.id}`}>
                <h2 className="text-lg font-bold leading-snug text-[--foreground] transition-colors hover:text-[--accent]">
                  {post.title}
                </h2>
              </Link>
            ))}
          ...
          {content.trim() &&
            (post.post_type === "article" ? (
              ...
              <Link href={`/posts/${post.id}`} className="block">
                <p className="line-clamp-6 whitespace-pre-wrap wrap-break-word text-sm leading-relaxed text-[--foreground]">
                  {content}
                </p>
                {content.length > 280 && (
                  <span className="mt-1 inline-block text-xs font-semibold text-[--accent] hover:underline">
                    See more
                  </span>
                )}
              </Link>
            ...
            (detail ? (
              ...
              <Link href={`/posts/${post.id}`} className="block">
                ...
                <img
                  src={post.image_url}
                  alt={post.title || "Post image"}
                  className="max-h-112 w-full rounded-lg border border-[--surface-secondary] object-cover"
                />
              </Link>
            ))}
        </Card.Content>
              ...
              <Card className="overflow-hidden border border-[--surface-secondary] shadow-2xl">
                ...
                <div className="flex items-center justify-end gap-2 border-t border-[--surface-secondary] px-5 py-3">
                  ...
                  <Button
                    ...
                    onPress={saveEdit}
                    isDisabled={
                      savingEdit || !draft.trim() || draft.trim() === content
                    }
